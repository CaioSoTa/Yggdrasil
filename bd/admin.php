...
if ($modulo == 'usuarios') {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['form_usuario'])) {
        $id = $_POST['id_usuario']; 
...
<nav class="navbar navbar-expand-lg navbar-dark bg-success mb-4 shadow-sm">
    <div class="container">
        <a class="navbar-brand" href="admin.php"> Admin Yggdrasil</a>
        <div class="collapse navbar-collapse">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0">
                <li class="nav-item">
                    <a class="nav-link <?= $modulo == 'usuarios' ? 'active fw-bold' : '' ?>" href="admin.php?modulo=usuarios">Usuários</a>
                </li>
                
            </ul>
            <a href="../pages/cladograma.html" class="btn btn-outline-light btn-sm">Voltar ao Site</a>
        </div>
    </div>
</nav>
...
<main class="container">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <?php if ($acao == 'listar'): ?>
                <a href="admin.php?modulo=usuarios&acao=criar" class="btn btn-success">Criar Novo Usuário</a>
            <?php endif; ?>
        </div>
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                ...
                ?>
                    ...
                    <form action="admin.php?modulo=usuarios" method="POST">
                        ...
                        <a href="admin.php?modulo=usuarios" class="btn btn-outline-danger">Cancelar</a>
                    </form>
                            ...
                            <tbody>
                                ...
                                if (mysqli_num_rows($result) > 0) {
                                    while($row = mysqli_fetch_assoc($result)) {
                                        $data_formatada = !empty($row['data_nascimento']) ? date('d/m/Y', strtotime($row['data_nascimento'])) : '-';
                                        ...
                                        echo "<td>
                                                <a href='admin.php?modulo=usuarios&acao=editar&id=" . $row['id'] . "' class='btn btn-warning btn-sm'>Editar</a>
                                                <a href='admin.php?modulo=usuarios&acao=deletar&id=" . $row['id'] . "' class='btn btn-danger btn-sm' onclick=\"return confirm('Tem certeza que deseja excluir?');\">Deletar</a>
                                              </td>";
                                        echo "</tr>";
                                    }
                                } else {
